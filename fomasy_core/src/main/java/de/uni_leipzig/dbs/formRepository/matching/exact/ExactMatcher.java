...
public class ExactMatcher extends Matcher {
  @Override
  public Long2FloatMap computeSimilarity(EncodedEntityStructure source, EncodedEntityStructure target,
      AggregationFunction function, float threshold, Pruning pruning) throws MatchingExecutionException {
    ...
    List<ExactThread> threadList = new ArrayList<ExactThread>();
    if (source.getObjIds().size()>target.getObjIds().size()) {
      ...
      for (int i=0;i<threadNumber;i++) {
        ...
        ExactThread tmpThread = new ExactThread (source, target, partObjIds, targetSet, function, threshold, this.getPropertyIds1(), this.getPropertyIds2(),
                pruning);
      ...
      for (int i=0;i<threadNumber;i++) {
        IntSet partObjIds = domObjIDsParts.get(i);
        ExactThread tmpThread = new ExactThread (source, target, sourceSet, partObjIds, function, threshold, this.getPropertyIds1(), this.getPropertyIds2(),
                pruning);
        threadList.add(tmpThread);
      }
    }

    for (ExactThread thread : threadList){
      thread.start();
    }
