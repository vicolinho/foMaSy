 ...
 */
public class RankingBasedMatcher extends Matcher {

  Logger log = Logger.getLogger(RankingBasedMatcher.class);
  ...
  public static final String WORD2VEC_MODEL = "word2vecModel";
  ...
  @Override
  public Long2FloatMap computeSimilarity(EncodedEntityStructure source, EncodedEntityStructure target, AggregationFunction function, float threshold, Pruning pruning) throws MatchingExecutionException {
    ...
    Word2Vec w2vmodel = null;
    if (this.getGlobalObjects().get(WORD2VEC_MODEL)!=null){
      w2vmodel = (Word2Vec) this.getGlobalObjects().get(WORD2VEC_MODEL);
    }
    ...
    List<RankingBasedThread> threadList = new ArrayList<>();
    if (source.getObjIds().size()>target.getObjIds().size()) {
      ...
      for (int i=0;i<threadNumber;i++) {
        ...
        RankingBasedThread tmpThread = new RankingBasedThread (source, target, partObjIds, targetSet, function,
                threshold, this.getPropertyIds1(), this.getPropertyIds2(),
                idfSourceMap, idfTargetMap, lookup, w2vmodel, wndSize);
      ...
      for (int i=0;i<threadNumber;i++) {
        IntSet partObjIds = domObjIDsParts.get(i);
        RankingBasedThread tmpThread = new RankingBasedThread (source, target, sourceSet, partObjIds,
                function, threshold, this.getPropertyIds1(), this.getPropertyIds2(),
                idfSourceMap, idfTargetMap, lookup, w2vmodel, wndSize);
        threadList.add(tmpThread);
      }
    }

    for (RankingBasedThread thread : threadList){
      thread.start();
    }
