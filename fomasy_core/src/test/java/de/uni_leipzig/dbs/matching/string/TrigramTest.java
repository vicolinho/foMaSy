package de.uni_leipzig.dbs.matching.string;
...
import de.uni_leipzig.dbs.formRepository.matching.execution.MatcherFactory;
...
public class TrigramTest {
  ...
  public void init(){
    ...
    //PropertyValue pv0 = new PropertyValue(2,"severre hemophillia");
    ...
    PropertyValue pv2 = new PropertyValue(1,"severre hemophillia");
    PropertyValue pv3 = new PropertyValue(2,"blabla");
    ge2.addPropertyValue(gp, pv2);
    ge2.addPropertyValue(gp, pv3);
    esv2.addEntity(ge2);
  ...
  public void testMatching () throws MatchingExecutionException {
    ...
    propsSrc.add(gp);
    MatchOperator mop = new MatchOperator(RegisteredMatcher.TRIGRAM_MATCHER,
            AggregationFunction.MAX, propsSrc, propsSrc, 0.65f);
    ExecutionTree tree = new ExecutionTree();
    tree.addOperator(mop);
    MatchManager mm = new MatchManagerImpl();
    Assert.assertEquals(19, ees.getTrigramIds()[0][0][0].length);
    ...
    for (EntityAnnotation ea: am.getAnnotations()){
      Assert.assertEquals((2f*17f)/(float)(19+21),ea.getSim(), 0.001);
    }
  }
}
